                    ...
                    postList.forEach { post ->
                        try {
                            BasePostsTable().insert { postTable ->
                                ...
                                if ((post.comments[0].comment.trim() == "") or (post.comments[0].comment.toLowerCase() == "verified")) {
                                    postTable[post_content] = post.comments[0].comment1
                                    postTable[post_music] =
                                            post.comments[0].comment1.getMusicName(records = post.comments[0].author)
                                } else {
